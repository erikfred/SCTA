function [smooth] = Z_godin(data,samplerate)
...
%adjust filter length according to data samplerate
filter=resample(filter,1:length(filter),fs*3600);
filter=filter/trapz(filter);
