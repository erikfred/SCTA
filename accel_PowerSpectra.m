...
if doWhat(1)
  ...
  if getData(1) 
    ...
    save pf_accel_PowerSpectra f1 f2a f2b p1 p2a p2b

  else
    load pf_accel_PowerSpectra
  ...
  xlim([0.01 10])
  ylim([1e-17 1e-11])
  print -djpeg pf_accelPowerSpectra.jpg
  print -dtiff pf_accelPowerSpectra.tiff -r300
end

%% Axial
if doWhat(2)
  startDate = datenum('Sept 5, 2018');
  endDate = datenum('Sept 10, 2018');
%   endDate = datenum('Sept 7, 2018');
  p1 = [];
  p2 = [];
  ...
  if getData(2) 
    for testDate = startDate:endDate-1
      scta = get_sctaDay('/users/wilcock/MyDrive/APL/SCTA-Share/OOI-SCTA/ParsedData',testDate);
      ...
      % Station PFO , network II, locatiion 00, BH1, BH2, BHZ - Guralp CMG-1T
      % trace = irisFetch.Traces(p.network.network,stationString,p.network.location,channelString,str1,str2);
      str1 = datestr(testDate, 'yyyy-mm-dd HH:MM:SS.FFF');
      str2 = datestr(testDate+1-1e-10, 'yyyy-mm-dd HH:MM:SS.FFF');
    %   trace = irisFetch.Traces('II','PFO','00','BHZ,BH1,BH2',str1,str2,'includePZ'); %'verbose');
...
%       axcc1 = [c1 c2 c3];
    ...
    save axial_accel_PowerSpectra f1 f2 p1 p2

  else
    load axial_accel_PowerSpectra
  ...
  xlim([0.01 10])
  ylim([1e-15 1e-10])
  print -djpeg axial_accelPowerSpectra.jpg
  print -dtiff axial_accelPowerSpectra.tiff -r300
end  
